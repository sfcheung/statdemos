...
between_two_lines <- function(x, y, a1, b1, a2, b2) {
    y01 <- a1 + b1 * x
    y02 <- a2 + b2 * x
    (y > max(y01, y02)) || (y < min(y01, y02))
  }

ui <- fluidPage(
  titlePanel("Hat Value of a Case When There Are Two Predictors (WORK-IN-PROGRESS. NOT READY)"),
  fluidRow(
    column(12,
      wellPanel(
  ...
  fluidRow(
    column(12,
      wellPanel(
          ...
          p("This version forces the version of the plot to 1:1.")
        ),
      wellPanel(
        p("Version 0.1.0"),
        p("Work-in-progress. Not ready."),
        p("The latest version of the code can be found at ",
          a("statdemos at GitHub",
            href="https://github.com/sfcheung/statdemos/tree/master/hatvalue2"),
          "."
          ),
        p("This app can be run directly by ",
          code("shiny::runUrl(\"https://github.com/sfcheung/statdemos/raw/master/apps/hatvalue2.zip\")")
          )
        )
      )
    )
