...
# UI
ui <- fluidPage(
    titlePanel("Sampling Distribution of Regression Coefficient"),
    fluidRow(
      column(12,
        wellPanel(
        ...
        wellPanel(
          ...
          p("The latest version of the code can be found at ",
            a("statdemos at GitHub",
              href = "https://github.com/sfcheung/statdemos/tree/master/bDistribution"),
              "."
            ),
          p("This app can be run directly by ",
            code("shiny::runUrl(\"https://github.com/sfcheung/statdemos/raw/master/apps/bDistribution.zip\")")
            ),
...
server <- function(input, output) {
    ...
    output$hist1 <- renderPlot({
        ...
        box_color2 <- rgb(255, 0, 0,
                          ...
                          alpha = 50,
                          names = "red_t")
        b_str <- formatC(b, digits = 2, format = "f")
        ...
        b_p_str <- formatC(b_p, digits = 3, format = "f")
        b_p05 <- b_p / 2
        b_p05_str <- formatC(b_p05, digits = 3, format = "f")
        sig <- ((b < cut_lo) || (b > cut_hi))
        legend(x = b,
               y = .50 * y_length,
               legend = paste0("B: ",
                               b_str),
